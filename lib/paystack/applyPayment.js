...
import { PLAN_PRICES, OPERATOR_PLAN_PRICES } from './plans'

// accountType picks which plan table, which account table ('profiles' vs
// 'operators'), and which update shape applies. Defaults to 'customer' so
// every existing caller (verify route, webhook, before this param existed)
...
export async function applyPaystackPayment({ reference, planId, email, amountSubunit, currency, source, accountType = 'customer' }) {
  ...
  const plans = isOperator ? OPERATOR_PLAN_PRICES : PLAN_PRICES
  const table = isOperator ? 'operators' : 'profiles'
  ...
  if (insertError) {
    if (insertError.code === '23505') {
      ...
      return { ok: true, alreadyProcessed: true, [isOperator ? 'operator' : 'profile']: existing || null }
    }
    return { ok: false, status: 500, error: 'Could not record transaction.' }
  }

  const updateFields = isOperator
  ...
  return { ok: true, alreadyProcessed: false, [isOperator ? 'operator' : 'profile']: updated }
}
