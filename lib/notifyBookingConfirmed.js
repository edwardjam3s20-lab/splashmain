...
async function notifyOperatorsBookingConfirmed(booking) {
  ...
  if (error) {
    console.error('notifyOperatorsBookingConfirmed: operators lookup failed:', error.message)
    return
  }
  if (!operators?.length) {
    // Not necessarily a bug -- but silent until now, which made this
    // exact case indistinguishable from a real failure. Logging the
    // location string that found no match makes it possible to tell,
    // from Vercel logs alone, whether this is an exact-match miss
    // against operators.wash_point (whitespace, casing, a renamed wash
    // point, etc.) rather than guessing.
    console.warn(`notifyOperatorsBookingConfirmed: no operators found for wash_point = "${booking.location}" (booking ${booking.id})`)
    return
  }
