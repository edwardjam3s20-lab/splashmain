...
import { operatorTrialDaysLeft } from './operatorAccess'
...
export function publicOperator(op) {
  ...
  return {
    ...
    // Present only once operator_freemium.sql has run — before that,
    // op.sub_status/created_at are undefined and these come back
    // undefined too, which the frontend treats as "no gate applies yet"
    // (same graceful-fallback pattern lib/loadOperator.js already uses).
    sub_status: op.sub_status,
    sub_plan: op.sub_plan ?? null,
    trial_days_left: op.created_at ? operatorTrialDaysLeft(op) : undefined,
  }
}
