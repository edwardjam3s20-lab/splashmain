...
export function verifyOperatorPassword(password, storedHash) {
  ...
  // Legacy SHA-256 with a pepper. This used to reuse SESSION_SECRET, which
  // meant rotating the JWT signing secret would silently break password
  // verification for any operator not yet auto-upgraded to bcrypt. Copy
  // SESSION_SECRET's current value into LEGACY_PASSWORD_PEPPER once, then
  // never change LEGACY_PASSWORD_PEPPER again — SESSION_SECRET can now be
  // rotated freely without affecting password verification.
  const secret = process.env.LEGACY_PASSWORD_PEPPER
  if (secret) {
