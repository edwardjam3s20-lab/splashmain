...
/** Must match lib/session.js fallback so hashes stay consistent. */
export function getSessionSecret() {
  return process.env.SESSION_SECRET || 'fallback_secret_32_chars_minimum!!'
}
...
function legacyHashCandidates(password) {
  const plain = String(password).trim()
  const secrets = [...new Set([getSessionSecret(), '', process.env.SESSION_SECRET || ''].filter((s) => s !== undefined))]
  const candidates = []
