...
function isEarnedBooking(b) {
  // Cash bookings never had money pass through the platform at all — the
  // customer paid the operator directly. Counting one as "earned" here
  // would tell computeOperatorOwed the platform owes the operator a
  // payout for money the operator already has in hand, i.e. pay them
  // twice for the same wash. status === 'completed' alone (the other
  // branch below) would otherwise still catch a completed cash booking
  // even with payment_status excluded from the OR list, so this has to
  // be its own early return, not just a missing value from the OR.
