...
/**
 * Same as sendPushToOperator but for a customer, keyed by email instead
 * of operator_id (see customer_reminders.sql for why — bookings.id and
 * profile identity in this project use email as the practical key, not
 * a uuid foreign key).
 */
export async function sendPushToCustomer(email, payload) {
  const supabase = getSupabaseAdmin()
  const { data: subs, error } = await supabase
  ...
  await Promise.allSettled(
    subs.map(async (sub) => {
      try {
        ...
        if (e.statusCode === 404 || e.statusCode === 410) {
          await supabase.from('customer_push_subscriptions').delete().eq('id', sub.id)
        } else {
          console.error('Push send failed:', sub.endpoint, e.statusCode, e.message)
        }
      }
